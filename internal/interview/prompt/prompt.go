// Package prompt builds the system prompt and message history for the
// interviewer LLM. Extracted from internal/interview to break the import
// cycle between backend and interview.
package prompt
...
// Builder constructs the system prompt and message history for the
...
// NewInterviewerPrompt returns a fresh Builder.
func NewInterviewerPrompt() *Builder {
	return &Builder{}
}
...
func (b *Builder) WithSystemInstructions() *Builder {
...
func (b *Builder) WithQuestion(q db.Question) *Builder {
...
func (b *Builder) WithTimeContext(elapsed, remaining time.Duration) *Builder {
...
func (b *Builder) WithCoachBriefing(ca *db.CoachAnalysis) *Builder {
...
func (b *Builder) WithTranscript(msgs []db.Message) *Builder {
	for _, m := range msgs {
		switch m.Role {
		case "interviewer":
...
func (b *Builder) Build() (string, []anthropic.MessageParam) {
	if len(b.msgs) == 0 {
