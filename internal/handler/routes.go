...
func NewHandler(b *backend.Backend, spaFS embed.FS, csrfKey []byte, secureCookies bool, rl *ratelimit.Limiter) (http.Handler, error) {
...
func RegisterRoutes(mux *http.ServeMux, b *backend.Backend, rl *ratelimit.Limiter) error {
	...
	mux.Handle("POST /api/auth/signup", rl.Middleware(Signup(b)))
	mux.Handle("POST /api/auth/login", rl.Middleware(Login(b)))
	mux.HandleFunc("POST /api/auth/logout", Logout(b))
	mux.HandleFunc("POST /api/auth/verify-email", VerifyEmail(b))
	mux.Handle("POST /api/auth/forgot-password", rl.Middleware(ForgotPassword(b)))
	mux.Handle("POST /api/auth/reset-password", rl.Middleware(ResetPassword(b)))
	...
	// Sessions
	mux.Handle("POST /api/sessions", requireAuth(CreateSession(b)))
	mux.Handle("GET /api/sessions", requireAuth(ListSessions(b)))
	mux.Handle("GET /api/sessions/{id}", requireAuth(GetSession(b)))
	mux.Handle("GET /api/sessions/{id}/ws", requireAuth(SessionWS(b)))

	// Evaluations
	mux.Handle("GET /api/sessions/{id}/evaluation", requireAuth(GetEvaluation(b)))
	mux.Handle("POST /api/sessions/{id}/evaluate", requireAuth(RetryEvaluation(b)))

	// Educator
	mux.Handle("GET /api/sessions/{id}/educator", requireAuth(GetEducatorAnalysis(b)))
	mux.Handle("POST /api/sessions/{id}/educator", requireAuth(RequestEducatorAnalysis(b)))

	// Coach
	mux.Handle("GET /api/coach/latest", requireAuth(GetCoachAnalysis(b)))
	mux.Handle("POST /api/coach/analyze", requireAuth(RequestCoachAnalysis(b)))

	// Billing
	mux.Handle("POST /api/billing/checkout", requireAuth(PostCheckout(b)))
	mux.Handle("POST /api/billing/portal", requireAuth(PostPortal(b)))
