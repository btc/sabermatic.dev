...
3. Persist the customer link. Because `UpdateUserStripeCustomerID` is tagged `:exec` and silently succeeds on zero rows affected, do the existence check first: call the existing `GetUserByID` query (returns `pgx.ErrNoRows` for missing user); surface that as "user not found". Then call `db.New(r.pool).UpdateUserStripeCustomerID(ctx, db.UpdateUserStripeCustomerIDParams{ID: userID, StripeCustomerID: pgtype.Text{String: cust.ID, Valid: true}})`. Two round-trips; the TOCTOU window between the SELECT and UPDATE is irrelevant for a dev tool. Do **not** change the sqlc tag on `UpdateUserStripeCustomerID` — that would alter the signature for production callers (`internal/backend/billing.go` uses it on the checkout path) and is out of scope.
