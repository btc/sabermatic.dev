# Cloud Bootstrap Script Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Create an interactive Python script that walks a solo operator through first-time GCP deployment — from zero to a running Cloud Run service with Stripe billing configured.

**Architecture:** Single-file Python 3 script (`scripts/cloud_bootstrap.py`) using only stdlib. Runs 9 phases sequentially with a JSON state file for resume. Supporting changes to Terraform configs, `.gitignore`, and `.env.example`.

**Tech Stack:** Python 3.10+ (stdlib only: `subprocess`, `json`, `shutil`, `pathlib`, `getpass`, `urllib.request`), gcloud CLI, Terraform, Docker, Stripe REST API.

**Spec:** `docs/superpowers/specs/2026-04-06-cloud-bootstrap-script-design.md`
...
|---|---|---|
| Create | `scripts/cloud_bootstrap.py` | Interactive bootstrap script — all 9 phases |
| Delete | `scripts/bootstrap.sh` | Superseded by cloud_bootstrap.py |
| Update | `terraform/secrets.tf` | Add `stripe-secret-key`, `stripe-webhook-secret` |
| Update | `terraform/cloud_run.tf` | Add Stripe secret refs + price ID env vars |
| Update | `.env.example` | Add Stripe env vars |
| Update | `.gitignore` | Add `.cloud_bootstrap_state` |

---

### Task 1: Update Terraform and config files for Stripe

**Files:**
- Modify: `terraform/secrets.tf:1-13`
- Modify: `terraform/cloud_run.tf:99-190`
- Modify: `.env.example`
- Modify: `.gitignore`

- [ ] **Step 1: Add Stripe secrets to `terraform/secrets.tf`**

In `terraform/secrets.tf`, add `"stripe-secret-key"` and `"stripe-webhook-secret"` to the `secret_ids` list:
...
- [ ] **Step 2: Add Stripe env vars to `terraform/cloud_run.tf`**

After the `OAUTH_GITHUB_CLIENT_SECRET` secret env block (around line 189), add:
...
```hcl
      ...
      # ---- Stripe price IDs (non-secret, set by bootstrap script) ----
      env {
...
STRIPE_PRO_PRICE_ID=
STRIPE_PACK_120_PRICE_ID=
STRIPE_PACK_300_PRICE_ID=
STRIPE_PACK_600_PRICE_ID=
```

- [ ] **Step 4: Add `.cloud_bootstrap_state` to `.gitignore`**

Append to the end of `.gitignore`:

```
# Cloud bootstrap state (resume file)
.cloud_bootstrap_state
```

- [ ] **Step 5: Verify Terraform config is valid**

Run: `cd /Users/btc/Projects/src/drill/terraform && terraform fmt -check . && echo "OK"`

Expected: OK (or apply `terraform fmt .` to fix formatting)

- [ ] **Step 6: Commit**

```bash
git add terraform/secrets.tf terraform/cloud_run.tf .env.example .gitignore
git commit -m "infra: add Stripe secrets and price ID env vars to Terraform"
```

---

### Task 2: Delete `scripts/bootstrap.sh`

**Files:**
- Delete: `scripts/bootstrap.sh`

- [ ] **Step 1: Delete the old bootstrap script**
...
```bash
git add -u scripts/bootstrap.sh
git commit -m "chore: remove bootstrap.sh, superseded by cloud_bootstrap.py"
```

---

### Task 3: Create `scripts/cloud_bootstrap.py` — scaffolding, state management, and Phase 1 (deps)

This is the largest task. It creates the script file with all infrastructure: argument parsing, colored output, state file management, phase runner, and the first phase.

**Files:**
- Create: `scripts/cloud_bootstrap.py`

- [ ] **Step 1: Create the script with scaffolding and Phase 1**

Create `scripts/cloud_bootstrap.py` with:
...
def phase_stripe(state: dict) -> None:
    ...
    if not stripe_state.get("products"):
        ...
        for i, prod in enumerate(products, 1):
            ...
            cents_str = prompt_value(f"  Price in cents", prod["default_cents"])
            cents = int(cents_str)
...
```bash
chmod +x scripts/cloud_bootstrap.py
```

- [ ] **Step 3: Verify script parses without errors**

Run: `python3 -c "import ast; ast.parse(open('scripts/cloud_bootstrap.py').read()); print('OK')"`
...
git commit -m "feat: add interactive GCP bootstrap script

9-phase interactive deployment: deps, auth, project, APIs/WIF,
Terraform, secrets, Stripe products/webhook, build, deploy.
Resume-safe via .cloud_bootstrap_state JSON file.

Replaces scripts/bootstrap.sh."
```
...
- [ ] **Step 1: Run the spec review agent**

Dispatch an Opus subagent to review `scripts/cloud_bootstrap.py` against `docs/superpowers/specs/2026-04-06-cloud-bootstrap-script-design.md`. The reviewer should verify:

1. All 9 phases from the spec are implemented
2. All secrets from the spec are covered
3. Stripe product creation matches the spec (4 products, correct defaults)
4. Webhook registration uses the correct 4 event types
5. State file format matches the spec
6. No hardcoded project IDs (should use state values)
7. `getpass.getpass()` used for secret input
8. Idempotency: all gcloud create commands handle "already exists"

- [ ] **Step 2: Fix any findings from the review**

- [ ] **Step 3: Final commit**

```bash
git add -u
git commit -m "fix: address review findings for cloud bootstrap script"
```
...
- The entire script is in a single task (Task 3) because it's one file with no test suite — this is an interactive ops script, not library code. TDD doesn't apply here.
- Task 1 (Terraform changes) must be done before Task 3 so the secret IDs in the script match what Terraform creates.
- Task 2 (delete old script) can run in parallel with Task 1.
- Task 4 (review) must run after Task 3.
