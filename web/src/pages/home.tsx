...
// ScoreSparkline — placeholder until per-session score_overall is available on Session type
function ScoreSparkline({ sessions }: { sessions: Session[] }) {
  const reviewed = reviewedSessions(sessions)
    .sort((a, b) => new Date(a.created_at).getTime() - new Date(b.created_at).getTime());
  if (reviewed.length < 3) return null;

  const data = reviewed.map((_, i) => ({ v: i + 1 }));
...
export default function Home() {
  ...
  const atConcurrentLimit = active.length > 0;
    ...
    <div className="space-y-6">
      ...
      {(isReturning || isActive) && (
        <div className="flex items-center gap-6">
          ...
          {isActive && <ScoreSparkline sessions={sessions} />}
        </div>
      )}
